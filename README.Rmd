...
using the argument: 'show_vista = F'. Then edit the colours with {magick} before
generating the rgl window with rayshader::plot_3d. 
...
supply an sf object (or alternatively an sf readable file path) to request an 
an area which is based off the extent of the sf object.
...
Finally it is also possible to provide your own elevation data, {rayvista} will
then automatically add an overlay to your data! Here is an example using the 
awesome [{terrainr}](https://docs.ropensci.org/terrainr/) package...
